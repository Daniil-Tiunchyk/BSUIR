...
public class AdminStudentsController {
    ...
    void edit(ActionEvent event) {
        ...
        if (userDTO != null && validateFields()) {
            ...
            userDTO.setLastName(surnameField.getText());
            userDTO.setPhoneNumber(telephoneField.getText());
            updateData(userDTO, "http://localhost:8080/api/users/" + userDTO.getUserId());
        } else {
            showWarningAlert("Пожалуйста, заполните все поля");
        }
    }
    ...
    void logout(ActionEvent event) {
        ...
        Node sourceNode = menuItem.getParentPopup().getOwnerWindow().getScene().getRoot();
    ...
    @FXML
    void initialize() {
        columnStudentID.setCellValueFactory(cellData -> new SimpleIntegerProperty(cellData.getValue().getUserId()));
