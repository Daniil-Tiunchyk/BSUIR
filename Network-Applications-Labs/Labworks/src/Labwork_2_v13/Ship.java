  ...
  public void run() {
    while (true) {
      if (totalContainers.get() <= 0) {
        printAndWrite(
          "Корабль с вместимостью " + capacity + " завершил работу."
        );
        break;
      }
      ...
      synchronized (lock) {
        ...
        printAndWrite(
          "В порту осталось " + totalContainers.get() + " контейнеров."
        );
      }
      try {
        ...
        trips++;
        printAndWrite(
          "Корабль с вместимостью " + capacity + " завершил рейс номер " + trips
        );
